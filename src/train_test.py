...
def learn(net, train, test, lr=1e-4, epoch=50):
    ...
    with torch.no_grad():
        for _, (x, y) in enumerate(test):
            x, y = x.view(-1, 1, 1000).float(), y.long()
            pred = torch.max(network(x), 1)[1].data
            pred = pred.cpu().numpy()
            test_y_pred += pred.tolist()
            test_y_true += y.tolist()
    
    test_score = f1_score(test_y_true, test_y_pred, average='micro')
    print("Test F1 Score: %.2f" % test_score)
    
    choice = input("Save or not?[y/n]")
    if choice == 'y':
        torch.save({'model_state_dict': net.state_dict()}, "../params/net_params.pkl")
    
    return np.array(loss_data), np.array(val_score), test_score, np.array(test_y_pred), np.array(test_y_true)

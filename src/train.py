...
class bi_lstm(nn.Module):
    def __init__(self, input_size=1000, hidden_size=100, num_layers=2, output_size=4, bidir=True):
...
class cnn_concat_lstm(nn.Module):
    def __init__(self):
        ...
        self.cnn = cnn_1d(256)  # Assuming the output size of CNN is 256
        self.lstm = bi_lstm(input_size=256, hidden_size=100, num_layers=2, output_size=4, bidir=True)

    def forward(self, x):
        cnn_features = self.cnn(x)  # Output shape: (batch_size, 1, 256)
        cnn_features = cnn_features.squeeze(1)  # Remove the channel dimension: (batch_size, 256)
        cnn_features = cnn_features.unsqueeze(1)  # Add the sequence dimension: (batch_size, 1, 256)

        lstm_out, _ = self.lstm.lstm1(cnn_features)
        lstm_out = self.lstm.linear(lstm_out[:, -1, :])
        out = self.lstm.sm(lstm_out)
...
def learn(net, train, test, lr=1e-4, epoch=50):
    ...
    for ep in range(epoch):
        ...
        for _, (x, y) in enumerate(train):
            x = x.view(-1, 1, 1000).float()  # Ensure input is float
            y = y.long()  # Ensure labels are long
            out = network(x)
        ...
        with torch.no_grad():
            for _, (x, y) in enumerate(test):
                x, y = x.view(-1, 1, 1000).float(), y.long()  # Ensure input is float and labels are long
                pred = torch.max(network(x), 1)[1].data
                pred = pred.cpu().numpy()
                y_pred += pred.tolist()
                y_true += y.tolist()
    ...
    return np.array(loss_data), np.array(val_score), np.array(y_pred), np.array(y_true)
...
def metric(y_pred, y_true):
    ...
    return classification_report(y_true, y_pred, target_names=['AFIB', 'GSVT', 'SB', 'SR'])
